...
func NewDynamicRouter(llm agent.LLM, toolRegistry *agent.ToolRegistry, opts ...DynamicRouterOption) *DynamicRouter {
	d := &DynamicRouter{
		llm:          llm,
...
func (d *DynamicRouter) Route(ctx context.Context, goal string) (*RouteResult, error) {
	intentFormat, err := agent.ResponseFormatFromStruct[IntentDetection]("intent_detection")
	if err != nil {
